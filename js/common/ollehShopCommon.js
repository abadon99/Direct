...
jQuery.extend({CommonObj : function(){
	...
	this.ajaxSend = function(cfg) {
		...
		if (cfg.isMsgBlock != undefined) {
		...
		} else if(cfg.isOneBlock != undefined) {
			...
			if(cfg.isBlock && $.isFunction($.loadBlock)) {
				if ($("#loading").length == 0) {
					$.loadBlock(cfg.isDim);
				}
			}
		...
		$.ajax({
			...
			, isMsgBlock : (cfg.isMsgBlock == undefined) ? true : cfg.isMsgBlock
...
function mZipCommon(selector, pageType, pageTitle, addrSvcType, limitType) {
	...
	var newViewYn = "N";
	if( ("wireless" == limitType) && (-1 < selector.indexOf("addrSearchPage")) ) {
		newViewYn = "Y";
	}

	var url = "/m/common/zipCodeView.do?pageType="+ pageType+"&pageTitle="+pageTitle+"&addrSvcType="+addrSvcType+"&limitType="+limitType+"&newViewYn="+newViewYn;
	return url;
}
...
function zipSearchCallBack(searchType,zipCd,addText,addDtlText,fulladdrText,searchTypeText, callBackFunction, layerYnText,limitType, callGbn, totalAddrText) {
	var newViewYn = "N";
	if("wlessDlvrAddrTxt" == totalAddrText) {
		newViewYn = "Y";
	}
...
/** 온라인마케팅시스템 omsf ORDER 호출 - 클릭 */
function fnOnmasOdrClick(prvObj, actionType, trackCode) {
	var odClickFnNm = "";
	if (trackCode) {
		odClickFnNm = "clickOrderLog('" +actionType+ "','" +trackCode+ "')";
	} else {
		odClickFnNm = "clickOrderLog('" +actionType+ "')";
	}
	fnOnmasOdrSender(prvObj, odClickFnNm);
}

/** 온라인마케팅시스템 omsf ORDER 호출 - 화면 */
function fnOnmasOdrView(pageCode, prodNo, sntyNo, giftProdCd, pgType) {

	var onmasExObj = $.extend(new $.CommonObj(),{});

	if (!pageCode) {
		return;
	}

	var varData = onmasExObj.getSerializedData({
			pageCode : pageCode
			, prodNo : prodNo
			, sntyNo : sntyNo
	...
	onmasExObj.ajaxSend({
		cache: false
		, url: '/common/getOnmasProdInfo.json'
		, data: varData
		, dataType: 'json'
		, async: false
		, isBlock: true
		...
		, successCall: function (jsonObj) {
			var result = jsonObj.onmasProdInfoBean;
			if (result != null) {
				var pupupYn = result.popupYn;
				var orderPageYn = result.orderPageYn;
				var fnNm = "Y" == pupupYn ? "pagePopLog()" : "pageLog()";
				var odrderFnNm = "pageOrderLog('" +pgType+ "')";
				fnOnmasOdrSender(result, fnNm);
				if ("Y" == orderPageYn && pgType) {
					result.pVal8 = result.sntyVal8;
					fnOnmasOdrSender(result, odrderFnNm);
				}
			}
		}
	});
...
/** 온라인마케팅시스템 omsf ORDER 호출 - Sender */
function fnOnmasOdrSender(prvObj, fnName) {
	try {
		var onmasExObj = $.extend(new $.CommonObj(),{});

		var pageNo = onmasExObj.nvl(prvObj.pageNo, o.m.muCd);
		var param4 = onmasExObj.nvl(prvObj.pVal4);
		var param5 = onmasExObj.nvl(prvObj.pVal5);
		var param6 = onmasExObj.nvl(prvObj.pVal6);
		var param7 = onmasExObj.nvl(prvObj.pVal7);
		var param8 = onmasExObj.nvl(prvObj.pVal8);

		var result = false;

		if (!pageNo || !param4 || !fnName) {
			return;
		}
		...
		o.p[0].prVal7 = param7;
		o.p[0].prVal8 = param8;
		(new Function("return omsf." + fnName))();
...
$(function() {
	...
	$(document).on("click", ".goTelOrderCounsel", function (e) {

		// controller가 선언되어 있지 않으면, 새로 생성 처리
		...
		var pageProdNo = controller.nvl($(this).attr("pageProdNo"), "");
		var plnDispYn = controller.nvl($(this).attr("plnDispYn"), "N");

		// 상품번호가 존재 시 페이지 호출 유형 조회
		var transPageCd = "P";
		try {
			if("" != pageProdNo) {
				controller.ajaxSend({
					cache: false
					, url: '/wire/getPageRturnType.json'
					, data: { prodNo: pageProdNo }
					, dataType: 'json'
					, async: false
					, isBlock: true
					, successCall: function (jsonObj) {
						transPageCd = jsonObj.transPageCd;
					}
				});
			}
		} catch(e) {
		...
		if("P" == transPageCd) {
			controller.createForm({
				...
				,method:"post"
				,action: url
				,target: "telCounselPoupView"
			});

			var inPageName = $(this).attr("inPageName");
			controller.attachHiddenElement("inPageName",inPageName);
			controller.attachHiddenElement("plnDispYn",plnDispYn);
			controller.attachHiddenElement("popYn","Y");

			// 팝업창 호출
			controller.formPopupSubmit("530", "800");
		} else {
			controller.createForm({
				id:"SubmitObj"
				,name:"SubmitObj"
			...
			controller.attachHiddenElement("inPageName",inPageName);
			controller.attachHiddenElement("plnDispYn",plnDispYn);
			controller.attachHiddenElement("popYn","N");
	...
	$(document).on("click", ".insInPageName", function (e) {
		// controller가 선언되어 있지 않으면, 새로 생성 처리
		if("undefined" == typeof controller) {
			var controller = $.extend(new $.CommonObj(),{});
			controller.init();
...
$(document).ready(function($) {
	$('.goTelOrderCounsel').each(function(index){
...
function trackClickTab(ClickCatName,ClickName,pageNameTemp) {
	try{
		...
		s.events="event3";
		s.eVar70=s.pageName = pageNameTemp;
		s.events=""
		s.t();
	}catch(e){
		return;
